...
export const AuthProvider = ({ children }) => {
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const error = new Error('App state check failed');
  ...
  const checkUserAuth = async () => {
    try {
      ...
      const currentUser = await khanyisaApi.auth.me();
      setUser(currentUser);
  ...
  const logout = (shouldRedirect = true) => {
    ...
    if (shouldRedirect) {
      ...
      khanyisaApi.auth.logout(window.location.href);
    } else {
      // Just remove the token without redirect
      khanyisaApi.auth.logout();
    }
  };

  const navigateToLogin = () => {
    // Use the SDK's redirectToLogin method
    khanyisaApi.auth.redirectToLogin(window.location.href);
  };
